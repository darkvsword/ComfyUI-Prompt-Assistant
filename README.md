...
* **VIDEO 接口兼容修复**：修复 VIDEO 类型端口输入（如 ComfyUI V3 原生视频节点）无法被视频反推节点识别导致报错「No video frames provided」的问题。
* **Ollama 自定义服务视频反推修复**：修复 Ollama 服务 ID 非默认值时，思维链控制参数未正确注入导致 Qwen3 系列 VLM 模型视频/图像反推返回空内容的问题。同时修复流式解析中 `thinking` 字段污染正文内容的逻辑错误。
