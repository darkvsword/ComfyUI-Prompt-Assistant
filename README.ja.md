...
* **VIDEO 入力互換性修正**: VIDEO 型ポート入力（ComfyUI V3 ネイティブビデオノードなど）がビデオキャプションノードで認識できず「No video frames provided」エラーになる問題を修正しました。
* **Ollama カスタムサービス VLM 修正**: Ollama サービス ID が非デフォルトの場合に `think` パラメータが注入されず、Qwen3 シリーズ VLM モデルが空の結果を返す問題を修正。また、ストリーミング解析で `thinking` フィールドが本文を汚染するバグも修正しました。
