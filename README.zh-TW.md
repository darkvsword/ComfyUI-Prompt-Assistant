...
* **VIDEO 介面相容修復**：修復 VIDEO 類型端口輸入（如 ComfyUI V3 原生視頻節點）無法被影片反推節點識別，導致「No video frames provided」的問題。
* **Ollama 自訂服務影片反推修復**：修復 Ollama 服務 ID 非預設值時，思維鏈控制參數未正確注入，導致 Qwen3 系列 VLM 模型返回空內容的問題。同時修復串流解析中 `thinking` 欄位污染正文的邏輯錯誤。
